...
export default function SearchPage() {
    ...
    const fetchInitialResults = useCallback(async () => {
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                if (isTimeout) {
                    ...
                    if (results.length === 0) {
                        ...
                        setError('Request took longer than expected. Showing available results.');
                    }
                } else {
            ...
            if (activeRequestRef.current && activeRequestRef.current.signal === controller.signal) {
                activeRequestRef.current = null;
            }
        }
    ...
    }, [searchField, fetchInitialResults, fetchCategories]);
    ...
    const applyFilters = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/paperByCategory?${queryParams.toString()}`, {
                signal: controller.signal
            });
            ...
            const data = await response.json();
            console.log('Category filter results received:', data ? data.length : 0, 'items');
            ...
            if (validData.length > 0) {
                ...
                setResults([]);
                setError('No papers found for selected categories. Please try different categories.');
            ...
            if (error.name === 'AbortError') {
                if (isTimeout) {
                    ...
                    // Don't clear results if we might have partial data
                    if (results.length === 0) {
                        setError('Filter request timed out. Try selecting fewer categories.');
                    } else {
                        // Still show partial results with a warning
                        setError('Request took longer than expected. Showing available results.');
                    }
    ...
    const fetchResults = useCallback(async () => {
        ...
        try {
            ...
            if (validData.length > 0) {
                ...
                setResults([]);
                setError(`No results found for "${searchTerm}" in ${searchField}. Please try different search terms.`);
            }
        } catch (error) {
            if (error.name === 'AbortError') {
                if (isTimeout) {
                    ...
                    // Don't clear results if we might have partial data
                    if (results.length === 0) {
                        setError('Search request timed out. Try a more specific search term.');
                    } else {
                        // Still show partial results with a warning
                        setError('Request took longer than expected. Showing available results.');
                    }
        ...
        try {
            ...
            return [...data].sort((a, b) => {
                if (criteria === 'publishDate') {
                    if (a.Published_Year && b.Published_Year) {
                        return parseInt(b.Published_Year) - parseInt(a.Published_Year);
                    } else {
                        return new Date(b.publishedDate || '2000-01-01') - new Date(a.publishedDate || '2000-01-01');
                    }
                } else if (criteria === 'recentlyAdded') {
                    return new Date(b.addedDate || '2000-01-01') - new Date(a.addedDate || '2000-01-01');
                }
                return 0;
            });
        } catch (error) {
            ...
            <CardContainer className="py-12 mb-20 flex-grow">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                    <div className="md:col-span-1">
                        <div className="bg-white rounded-lg shadow-lg p-6 border border-gray-200 border-t-4 sticky top-40 hover:shadow-xl transition-all" style={{ borderTopColor: '#CDAE5E' }}>
                            <h2 className="text-xl font-semibold mb-4" style={{ color: '#CDAE5E' }}>
                                        ...
                                        <ul className="space-y-2">
                                            {categories.map((category, index) => (
                                                <li key={index} className="flex items-center">
                                                    ...
                                                    />
                                                    <label htmlFor={`checkbox-${category}`} className="text-gray-700 cursor-pointer text-sm">
                                                        {category}
                                                    </label>
                                                </li>
                                            ))}
                        ...
                        <div className="bg-white rounded-lg shadow-lg p-5 mb-6 border border-gray-200 hover:shadow-xl transition-all">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center bg-gray-50 rounded-lg p-3 mb-4">
                                ...
                                <div className="text-gray-600">
                                    Showing {getPaginatedResults().length} of {sortedResults.length} results
                                </div>
                            </div>
                            {error && (
                            ...
                            <div className="space-y-4 mb-8 min-h-[300px]">
                                {loading ? (
                                    Array.from({ length: 3 }).map((_, index) => (
                                        <div key={index} className="border border-gray-200 rounded-lg p-5 bg-white animate-pulse">
                                            ...
                                            >
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-3">
                                                    <div>
                                                        ...
                                                        <span className="text-gray-800">{item.Authors || 'Unknown'}</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-gray-600 font-medium">Journal:</span>{' '}
                                                        <span className="text-gray-800">{item.Journal || 'Unknown'}</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-gray-600 font-medium">Year:</span>{' '}
                                                        <span className="text-gray-800">
                                                            {item.Published_Year || (item.publishedDate ? new Date(item.publishedDate).getFullYear() : 'Unknown')}
                                                        </span>
                                                    </div>
                                                </div>
                                                {visibleAbstractId === absoluteIndex ? (
                                                    <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                                                        <h4 className="font-medium mb-2 text-gray-800">Abstract</h4>
                                                        <p className="text-gray-700">{item.Abstract || item.content || 'No abstract available'}</p>
                                                    ...
                                                    <div className="text-sm mt-2" style={{ color: '#CDAE5E' }}>
